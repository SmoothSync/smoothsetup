...
import com.smoothsync.smoothsetup.microfragments.PasswordMicroFragment;
 ...
 * @author Marten Gajda
 */
public final class EnterPassword implements MicroWizard<Account>
{
    private final MicroWizard<AccountDetails> mNext;


    public EnterPassword(MicroWizard<AccountDetails> next)
    {
        mNext = next;
    }
    ...
    {
        return new PasswordMicroFragment(account, mNext);
    }
    ...
    private final static class WizardBox extends AbstractSingleBox<MicroWizard<AccountDetails>, MicroWizard<Account>>
    {

        WizardBox(MicroWizard<AccountDetails> next)
        {
            super(next, EnterPassword::new);
        }
